...
class TransactionState:
    ...
    def takeSnapshot(self, rel, attr, reads):
        # YOUR CODE HERE
        for id in reads:
            self.snapshot[(attr, id)] = rel.getTuple(id).getAttribute(attr)

    def getAttribute(self, rel, id, attr):
        # modify for SNAPSHOT_ISO
        if self.mode == TransactionState.SNAPSHOT_ISO:
            # YOUR CODE HERE
            for (r, i, a, newval) in self.writeBuffer:
                if r == rel and id == i and attr == a:
                    return newval
            return self.snapshot[(attr,id)]
    ...
    def setAttribute(self, rel, id, attr, newval):
        ...
        if self.mode == TransactionState.SNAPSHOT_ISO:
            # YOUR CODE HERE
            self.writeBuffer.append((rel,id,attr,newval))
            return 
        tup = rel.getTuple(id)
    ...
    def commitTransaction(self):
        ...
        if self.mode == TransactionState.SNAPSHOT_ISO:
            # YOUR CODE HERE
            for (r, i, a, newval) in self.writeBuffer:
                snapVal = self.snapshot[(a,i)] 
                currVal = r.getTuple(i).getAttribute(a)
                if(snapVal != currVal):
