...
class GroupByAggregate(Operator):
	...
	def update_aggregate(aggregate_function, current_aggregate, new_value):
		...
		elif aggregate_function == GroupByAggregate.AVERAGE or aggregate_function == GroupByAggregate.MEDIAN or aggregate_function == GroupByAggregate.MODE or aggregate_function == GroupByAggregate.STDEV:
			if current_aggregate is None:
	...
	def final_aggregate(aggregate_function, current_aggregate):
		if aggregate_function in [GroupByAggregate.COUNT, GroupByAggregate.SUM, GroupByAggregate.MIN, GroupByAggregate.MAX]:
			...
			if current_aggregate is None:
				...
				total = 0
				for num in current_aggregate:
					total += num
				return total/len(current_aggregate)
			...
			if current_aggregate is None:
				...
				if n%2 == 0:
					return (current_aggregate[n // 2] + current_aggregate[(n // 2) - 1])/2
				else:
					return current_aggregate[n // 2]
			...
			if current_aggregate is None:
				...
				for num in freq:
					if freq[num] > count:
						...
						mode = num
				return mode
		elif aggregate_function == GroupByAggregate.STDEV:
			if current_aggregate is None:
				return None
			else:
				sample_mean = 0
				...
				for num in current_aggregate:
					sample_mean += num
				sample_mean = sample_mean/len(current_aggregate)
				
				std = 0
				for num in current_aggregate:
					std += (num - sample_mean)**2
				return round((std/(n-1))**(1/2),3)
		else:
			raise ValueError("No such aggregate")
...
# Implement a NaturalJoin operator. Note that tuple order is not defined on any Join output.
class NaturalJoin(Operator):
	...
	# We will use 'yield' to simplify the code
	def get_next(self):
			...
			while ptr_l < len(left_input) and ptr_r < len(right_input):
				...
				while ptr_r < len(right_input) and right_input[ptr_r].getAttribute(self.right_attribute) <= l_attr:
					if right_input[ptr_r].getAttribute(self.right_attribute) == l_attr:
						...
						output = [None] * len(self.left_child.relation.schema)
						output.extend(list(right_input[ptr_r].t))
						yield Tuple(None, output)							
					ptr_r += 1
				...
				if match == False:
					for l in set_L:
						output = list(l.t)
						output.extend([None] * len(self.right_child.relation.schema))
						yield Tuple(None, output)
...
class SetMinus(Operator):
	...
	def get_next(self):
		...
		if (self.left_child.get_next() is None):
			...
			yield Tuple(None, (self.left_child.get_next()))
		
		if(self.keep_duplicates == True):
			for r in self.right_child.get_next():
				right_side.append(r.t)

			for l in self.left_child.get_next():
				left_side.append(l.t)
			...
			for l in left_side:
				yield Tuple(None, (list(l)))
		else:
			for r in self.right_child.get_next():
				set_table.add(r.t)
			
			for l in self.left_child.get_next():
				if l.t not in set_table:
					yield Tuple(None, (list(l.t)))
