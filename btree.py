	...
	def redistributeWithBlock(self, otherBlock, kprime):
		...
		if self.isUnderfull():
			...
			# move the last ptr, key from the predecessor into the beginning of the successor block
				m = [otherBlock.keysAndPointers[-3], otherBlock.keysAndPointers[-2]]
				self.keysAndPointers.insert(0, m[1])
				self.keysAndPointers.insert(0, m[0])
				del otherBlock.keysAndPointers[-3]
			...
			# update the moved ptr's parent by calling (ptr.getBlock().parent = ???)
				m = [otherBlock.keysAndPointers[-1], otherBlock.keysAndPointers[-2]]
				self.keysAndPointers.insert(0, kprime)
				self.keysAndPointers.insert(0, m[0])
				m[0].getBlock().parent = Pointer(self.blockNumber)
				del otherBlock.keysAndPointers[-1]
				del otherBlock.keysAndPointers[-1]
				return m[1]
		else:
			...
			# symmetrical to Case 1
				m = [self.keysAndPointers[0],  self.keysAndPointers[1]]
				otherBlock.keysAndPointers.insert(-1, m[0])
				otherBlock.keysAndPointers.insert(-1, m[1])
				del self.keysAndPointers[0]
				del self.keysAndPointers[0]
				return self.keysAndPointers[1]
			else:
			# Case 4: predecessor is underfull and is not leaf
				m = [self.keysAndPointers[0],kprime]
				otherBlock.keysAndPointers.insert(len(otherBlock.keysAndPointers), m[1])
				otherBlock.keysAndPointers.insert(len(otherBlock.keysAndPointers), m[0])
				m[0].getBlock().parent = Pointer(otherBlock.blockNumber)
				del self.keysAndPointers[0]
				del self.keysAndPointers[0]
				return self.keysAndPointers[1]
